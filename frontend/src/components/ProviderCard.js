...
const ProviderCard = ({ provider }) => {
  ...
  const handleClick = () => {
    // Store provider data in sessionStorage and navigate to a simple detail view
    sessionStorage.setItem('selectedProvider', JSON.stringify(provider));
    window.location.href = '/provider-detail';
  };

  return (
    <div onClick={handleClick} style={{ cursor: 'pointer' }} data-testid={`provider-card-${provider.provider_id}`}>
